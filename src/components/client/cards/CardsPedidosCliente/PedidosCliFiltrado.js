...
function PedidosCliFiltrado(props) {
    ...
    useEffect(() => {
        const getItens = async () => {
            const res = await fetch(
                `https://gemeos-server.herokuapp.com/cliente/pedidos/${idCli}?_page=1&_limit=3`,{
                method: 'GET',
    ...
    const fetchItens = async () => {
        const res = await fetch(
            `https://gemeos-server.herokuapp.com/cliente/pedidos/${idCli}?_page=${page}&_limit=3`
        );
        const data = await res.json();
        return data;
    };
                ...
                {items
                    .filter((val) => {
                        if (busca === "") {
                            ...
                            val.Data_Pedido.includes(busca.toLowerCase())
                        ) {
                            return val
                        }
                    })
...
export default PedidosCliFiltrado;
